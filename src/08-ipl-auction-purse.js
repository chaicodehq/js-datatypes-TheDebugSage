...
export function iplAuctionSummary(team, players) {
  ...
  const totalSpent = players.reduce(function (sum, player) {
  ...
  const costliestPlayer = players.reduce(function (max, player) {
    return player.price > max.price ? player : max;
  });

  
  const cheapestPlayer = players.reduce(function (min, player) {
  ...
  const byRole = players.reduce(function (acc, player) {
    if (!acc[player.role]) {
      acc[player.role] = 0;
    }
    acc[player.role]++;
