...
const passengers = pnrData.passengers.map((p) => {
    ...
    } else if (typeof p.current === "string" && (p.current.startsWith("B") || p.current.startsWith("S"))) {
    statusLabel = "CONFIRMED";
    } else if (typeof p.current === "string" && p.current.startsWith("WL")) {
    statusLabel = "WAITING";
    } else if (typeof p.current === "string" && p.current.startsWith("RAC")) {
    statusLabel = "RAC";
    }
...
const allConfirmed = passengers.every(p => p.statusLabel === "CONFIRMED");
